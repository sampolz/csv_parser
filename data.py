...
class Data:
    ...
    def read(self, filepath):
        ...
        TODO:
        1. Set or update your `filepath` instance variable based on the parameter value. X
        2. Open and read in the .csv file `filepath` to set `self.data`. X
        ...
        potential variable types).
            - Numeric data: Store all values as floats. X
            - Categorical data: Store values as ints in your list of lists (self.data). Maintain the mapping between the
            int-based and string-based coding of categorical levels in the self.cats2levels dictionary. X
        All numeric and categorical values should be added to the SAME list of lists (self.data). X
        3. Represent `self.data` (after parsing your CSV file) as an numpy ndarray. To do this: X
            - At the top of this file write: import numpy as np X
            - Add this code before this method ends: self.data = np.array(self.data) X
        4. Be sure to set the fields: `self.headers`, `self.data`, `self.header2col`, `self.cats2levels`. X
        ...
        headerList = []
        headerToColDict = {}
        headerToUncleanedColDict = {}
        headerToType = {}

        dataTypesLine = lines[1].strip().split(',')
        headersLine = lines[0].strip().split(',')
        for i in range(len(dataTypesLine)):
            numHeaders = len(headerToColDict.keys())
            dataType = dataTypesLine[i].strip().lower()
            if(dataType == 'numeric' or dataType == 'categorical'):
                curHeader = headersLine[i].strip()
                headerList.append(curHeader)
                headerToUncleanedColDict[curHeader] = i
                headerToColDict[curHeader] = numHeaders
                headerToType[curHeader] = dataType

        # print(headerToColDict)
        # print(headerToType)
        # print(headerList)

        if(len(headerList) == 0):
            raise ValueError("No headers and/or category types provided. Please check lines 1 and 2 of CSV")
        
        self.headers = headerList
        self.header2col = headerToColDict
        headerToData = {}
        dataArray = []

        for line in lines[2:]:
            self.numDataPoints += 1
            cleanedLineArray = line.strip().split(',')
            templist = []
            for header in headerList:
                columnIndex = headerToUncleanedColDict[header]
                item = cleanedLineArray[columnIndex].strip()
                itemType = headerToType[header]
                ...
                if(header not in headerToData and itemType == 'categorical'):
                    headerToData[header] = []
                if(itemType == 'categorical' and item not in headerToData[header]):
                    headerToData[header].append(item)

                if(itemType == 'categorical'):
                    index = headerToData[header].index(item)
                    item = index
                else:
                    item = float(item)
    ...
    def __str__(self):
        ...
        '''
        headerString = f"{self.filepath} ({self.numDataPoints}x{len(self.headers)})\nHeaders:\n  {'  '.join(self.headers)}\n"
        divider = "-------------------------------\n"
        showingString = f"Showing first {min(5, self.numDataPoints)}/{self.numDataPoints} rows.\n"

        dataRowsString = ""
        for i in range(min(5, self.numDataPoints)):
            rowData = self.data[i]
            rowArray = []
            for data in rowData:
                if(isinstance(data,float)):
                    ...
                    val = str(data)
                rowArray.append(val)
            rowString = "  ".join(rowArray)
            dataRowsString += rowString + '\n'

        full_str = divider + headerString + divider + showingString + dataRowsString + divider
    ...
    def get_header_indices(self, headers):
        ...
        for header in headers:
            list.append(self.header2col[header])
        return list
